...
import net.openconnect_vpn.android.core.UserDialogUi;
import net.openconnect_vpn.android.core.CredentialStore;
import net.openconnect_vpn.android.core.AuthPersist;
...
public class AuthFormHandler extends UserDialog
	...
	private CheckBox saveDomain = null;
	...
	public AuthFormHandler(SharedPreferences prefs, LibOpenConnect.AuthForm form, boolean authgroupSet,
		...
		loginDomain = CredentialStore.loginDomain(getStringPref("server_address"));
		if (loginDomain == null) {
			loginDomain = "";
		}

		String s = getStringPref("batch_mode");
		if (s.equals("") || s.equals("empty_only")) {
	...
	/** Profile-level credentials set when creating a VPN (batch-mode friendly). */
	private String savedCredential(LibOpenConnect.FormOpt opt, boolean[] usedUser, boolean[] usedPass) {
		Context app = ProfileManager.getAppContext();
		boolean wantShared = getBooleanPref("use_shared_login")
				|| (!loginDomain.equals("") && app != null
						&& CredentialStore.hasRealm(app, loginDomain));

		if (opt.type == LibOpenConnect.OC_FORM_OPT_PASSWORD && !usedPass[0]) {
			usedPass[0] = true;
			if (wantShared && !loginDomain.equals("") && app != null) {
				CredentialStore.RealmSecret realm = CredentialStore.readRealm(app, loginDomain);
				if (realm != null && !realm.isEmpty()) {
					AuthPersist.markUsedShared();
					return realm.password;
				}
			}
			String pass = getStringPref("login_password");
			if (!pass.equals("")) {
				return pass;
			}
			return "";
		}
		if (opt.type == LibOpenConnect.OC_FORM_OPT_TEXT && !usedUser[0]) {
			usedUser[0] = true;
			String user = getStringPref("login_username");
			if (!user.equals("")) {
				return user;
			}
			if (wantShared && !loginDomain.equals("") && app != null) {
	...
	private String resolveTextDefault(LibOpenConnect.FormOpt opt, boolean[] usedUser, boolean[] usedPass) {
		String defval = noSave ? "" : getStringPref(formPfx + getOptDigest(opt));
		if (defval.equals("") && !noSave) {
			defval = savedCredential(opt, usedUser, usedPass);
		}
	...
	private LinearLayout newHorizLayout(String label) {
		...
		int gap = dp(10);
		ll.setPadding(0, gap / 2, 0, gap);
		fixPadding(ll);

		TextView tv = new TextView(mContext);
		tv.setText(label);
		tv.setTextColor(color(R.color.myoc_text_muted));
		tv.setTextSize(12);
	...
	private LinearLayout newTextBlank(LibOpenConnect.FormOpt opt, String defval) {
		...
		tv.setText(defval);
		tv.setTextColor(color(R.color.myoc_text));
		tv.setHintTextColor(color(R.color.myoc_text_muted));
	...
	private LinearLayout newDropdown(final LibOpenConnect.FormOpt opt, int selection) {
		...
		sp.setBackgroundResource(R.drawable.bg_edit_text);
	...
	private CheckBox newSavePasswordView(boolean isChecked) {
		...
		cb.setTextColor(color(R.color.myoc_text));
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			cb.setButtonTintList(mContext.getResources().getColorStateList(R.color.myoc_checkbox_tint, mContext.getTheme()));
		}
		fixPadding(cb);
		return cb;
	}

	private CheckBox newSaveDomainView(boolean isChecked) {
		CheckBox cb = new CheckBox(mContext);
		cb.setText(mContext.getString(R.string.save_for_domain, loginDomain));
		cb.setChecked(isChecked);
		cb.setTextColor(color(R.color.myoc_text));
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			cb.setButtonTintList(mContext.getResources().getColorStateList(R.color.myoc_checkbox_tint, mContext.getTheme()));
	...
	private void captureAuthCredentials() {
		String username = "";
		String password = "";
		for (LibOpenConnect.FormOpt opt : mForm.opts) {
			...
			if (opt.type == LibOpenConnect.OC_FORM_OPT_TEXT) {
				if (opt.userData instanceof TextView) {
					username = ((TextView) opt.userData).getText().toString();
				} else if (opt.value != null) {
					username = opt.value;
				}
			} else if (opt.type == LibOpenConnect.OC_FORM_OPT_PASSWORD) {
				if (opt.userData instanceof TextView) {
					password = ((TextView) opt.userData).getText().toString();
				} else if (opt.value != null) {
					password = opt.value;
				}
			}
		}
		boolean savePassChecked = savePassword == null || savePassword.isChecked();
		boolean saveDomainChecked = saveDomain != null && saveDomain.isChecked();
		/* Batch / early-return has no checkboxes — keep domain share if already opted in. */
		if (saveDomain == null && !loginDomain.equals("") && getBooleanPref("use_shared_login")) {
			saveDomainChecked = true;
		}
		AuthPersist.capture(username, password, saveDomainChecked, savePassChecked);
	}
	...
	public Object earlyReturn() {
		...
		boolean[] usedUser = new boolean[] { false };
		boolean[] usedPass = new boolean[] { false };
		for (LibOpenConnect.FormOpt opt : mForm.opts) {
			if ((opt.flags & LibOpenConnect.OC_FORM_OPT_IGNORE) != 0) {
				continue;
			}
			...
			case LibOpenConnect.OC_FORM_OPT_PASSWORD:
			case LibOpenConnect.OC_FORM_OPT_TEXT:
				String defval = resolveTextDefault(opt, usedUser, usedPass);
				if (defval.equals("")) {
					return null;
				}
				opt.value = defval;
	...
	public void onStart(Context context) {
		...
		for (LibOpenConnect.FormOpt opt : mForm.opts) {
			...
			case LibOpenConnect.OC_FORM_OPT_PASSWORD:
				...
				/* falls through */
			case LibOpenConnect.OC_FORM_OPT_TEXT:
				defval = resolveTextDefault(opt, usedUser, usedPass);
				if (defval.equals("")) {
		...
		if (hasPassword && !noSave) {
			...
			boolean saveDomainDefault = getBooleanPref("use_shared_login");
			if (!saveDomainDefault && !loginDomain.equals("")) {
				Context app = ProfileManager.getAppContext();
				if (app != null && CredentialStore.hasRealm(app, loginDomain)) {
					saveDomainDefault = true;
				}
			}
			savePassword = newSavePasswordView(savePass);
			v.addView(savePassword);
			if (!loginDomain.equals("")) {
				saveDomain = newSaveDomainView(saveDomainDefault);
				v.addView(saveDomain);
			}
		}
		...
		if (!hasUserOptions || (mAllFilled && (batchMode == BATCH_MODE_EMPTY_ONLY
				|| batchMode == BATCH_MODE_ENABLED))) {
			saveAndStore();
	...
	public void onStop(Context context) {
		...
		if (mAlert != null) {
			/* Do not capture credentials here — only after explicit OK via onDismiss. */
			mAlert.setOnDismissListener(null);
			mAlert.dismiss();
			mAlert = null;
		}
		/* Only abort auth when the host Activity is going away for good.
		 * Temporary onPause must keep waitForResponse alive so resume can re-show. */
