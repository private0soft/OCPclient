...
public abstract class UserDialog {
	...
	public Object waitForResponse() {
		synchronized (this) {
			...
			return mResult == EMPTY ? null : mResult;
		}
	}

	protected void finish(Object result) {
		synchronized (this) {
			/* Must notify even after onStop(); otherwise the VPN thread
			 * waits forever and Status stays on Connecting. */
			if (mResult != null) {
				return;
			}
			mResult = result != null ? result : EMPTY;
			this.notifyAll();
		}
	}
