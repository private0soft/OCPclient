...
public class KeepAlive extends BroadcastReceiver {
	...
	private static final int WAKELOCK_MS = 15000;
	...
	/* Dummy name: the query stays inside the tunnel DNS and never hits a public resolver. */
	private String mDNSHost = "vpn-keepalive";
		...
		mWorkerHandler.post(new Runnable() {
			...
			public void run() {
				...
				mMainHandler.post(new Runnable() {
					...
					public void run() {
						if (!result) {
							Log.i(TAG, "KeepAlive: probe failed; retrying at the same interval");
						}
	...
	private void scheduleNext(Context context, int delayMs) {
		...
		am.setAndAllowWhileIdle(AlarmManager.ELAPSED_REALTIME_WAKEUP, trigger, mPendingIntent);
	}

	public void start(Context context) {
		if (mBaseDelayMs == 0 || mDNSServer == null || mDNSServer.length() == 0) {
			Log.i(TAG, "KeepAlive: skipped (no tunnel DNS; DPD will keep the session)");
			return;
		}
